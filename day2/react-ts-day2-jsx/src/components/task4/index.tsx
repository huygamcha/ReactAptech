...
function ProductItem({ value }: { value: ProductItem }) {
    ...
    return (
        <div className={styles.product_item}>
            {(discount < 100) ? <label className={styles.product_discount}>-{discount}%</label> : ''}
            ...
            <div className={styles.product_des}>
                ...
                <div>
                    {value.promoPrice ?
                        <div>
                            <span className={styles.price_current}>{`${value.promoPrice}\u0111`}</span>
                            <del className={styles.price_pre}>{`${value.price}\u0111`}</del>
                        </div>
                        :
                        <div>
                            <span className={styles.price_current}>{`${value.price}\u0111`}</span>
                        </div>
                    }
                </div>
            </div>
